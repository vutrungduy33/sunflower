...
管理端说明：当前房型后台管理接口（`/api/admin/rooms*`、`/api/admin/room-prices`、`/api/admin/room-inventory`）要求携带 `Authorization: Bearer <admin-token>`；未携带时返回“请先登录管理端”，token 无效时返回“管理端登录态无效”。
