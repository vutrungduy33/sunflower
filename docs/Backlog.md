...
- [ ] S18 真实微信支付/退款与资金流水
- [x] S19 生产入口与微信发布阻塞项收口（2026-03-24：prod 配置切换到 ECS 本地 `.env.prod`，新增 `.release.env`、bootstrap/常规发布拆分、宿主机 Nginx HTTPS 入口模板、workflow_dispatch 回滚与小程序默认 HTTPS 域名配置位）
