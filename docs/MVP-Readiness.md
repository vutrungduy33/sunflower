...
| Admin operations path | Core pages and tests exist for auth, room, price/inventory, and order management; manual QA ledger now exists. Round 108 added `node scripts/check_admin_web_entry_readiness.js` and verified the temporary HTTP/IP admin entry, `/healthz`, and `/api/health` without login or data mutation. | Partially verified | Run `node scripts/check_admin_web_entry_readiness.js` before manual QA, then `node scripts/check_admin_web_manual_qa.js --strict` against deployed admin web after recording safe evidence. |
| Deployment | Round 82 changed the workflow so a GitHub hosted job packages the deployment bundle and ECS deploy jobs download workflow artifacts instead of running `actions/checkout`. Round 87 changed backend/admin image builds to plain Docker CLI build/push/save. Round 89 fixed the nonprod/mock overlay so `.env.prod` remains the ECS-owned base env. Round 90 fixed the WeChat payment LOB schema drift with Flyway V8 and restored backend health. Round 91 backend-only `deployment_lane=nonprod-mock-payment` run `27112433529` for HEAD `d10d11e` passed strict readiness, backend-only target detection, deployment bundle packaging, backend build/GHCR push/image export/upload, ECS-2 bundle sync, backend image download/load, nonprod/mock lane validation, backend recreation, and backend health wait. Round 98 checked the active workflow again and confirmed the latest successful run is still `27112433529`; later docs/checker commits did not touch any deploy-trigger paths, so they did not create a new workflow run. Post-deploy smoke passed 7/7 and backend 8080 exposure passed 5/5. Production lane still fails before backend recreation until real WeChat Pay config is provisioned. | Partially verified | Treat Round 91 as current-backend reduced-scope deploy evidence only. Production-like completion still requires real WeChat Pay variables/key files, HTTPS legal domain, real payment/refund evidence, and external miniapp/admin QA. |
| Security / compliance | Secrets are local/ECS-owned; `.secrets/` ignored. Round 58 closed external backend `8080` by changing ECS-2 production `BACKEND_BIND_HOST` from `0.0.0.0` to `172.25.121.83`, recreating `sunflower-backend`, and verifying public 8080 is unusable while ECS-1 private upstream remains healthy. User-provided备案 domains now include `xiangrikui.cloud` and `sunflower.cloud`. Round 107 reran read-only domain checks: `sunflower.cloud` has a trusted GoDaddy certificate valid until 2026-10-04 but `/api/health` returns an HTML lander redirect instead of backend health JSON; `xiangrikui.cloud`, `api.sunflower.cloud`, and `api.xiangrikui.cloud` still fail TLS/SNI handshake. Free DV certificates such as Let's Encrypt can satisfy the technical HTTPS shape if DNS points to ECS-1, the chain is trusted, TLS is modern, renewal is automated, the API host returns backend health, and the exact hostname is configured in the WeChat miniapp backend as a legal request domain. | Partially ready | Keep backend `8080` private after redeploys; point the chosen备案 API hostname to ECS-1, install/renew a trusted certificate, make `/api/health` return backend health JSON, then use `node scripts/check_miniapp_https_domain.js https://<api-domain>` and configure the WeChat legal request domain before recording evidence. |
...
- Round 100 production read-only audit
  ...
  real-payment config issues. No push, deploy, workflow dispatch, Nginx reload,
  ECS mutation, firewall/security-group mutation, payment/refund action, or
  live QA data mutation was performed.
...
- Round 98 rechecked GitHub Actions after later docs/evidence commits and found
  no newer `deploy-backend.yml` runs than `27112433529`. The post-Round-91
  commits only changed docs and miniapp checker scripts, and they did not touch
  the deploy path filters in `.github/workflows/deploy-backend.yml`, so they
  did not trigger ECS deployment.
