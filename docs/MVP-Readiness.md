...
| Deployment | Round 82 changed the workflow so a GitHub hosted job packages the deployment bundle and ECS deploy jobs download workflow artifacts instead of running `actions/checkout`. Run `26803892859` proved ECS-2 can download/extract/sync that bundle and download/load the backend image artifact, then failed production env validation because `WECHAT_PAY_MCH_ID` is missing. Round 87 changed backend/admin image builds to plain Docker CLI build/push/save; push run `26931880619` proved the hardened build path plus ECS backend artifact download/load, then failed at production `WECHAT_PAY_MCH_ID is required`. Round 88 backend-only `deployment_lane=nonprod-mock-payment` run `26932183311` proved strict readiness, backend-only dispatch, backend build/push/export/upload, ECS bundle/image artifact download/load, and nonprod lane guard, but failed during local backend deploy because MySQL app credentials from `.env.prod` could not access database `sunflower` as user `sunflower`. Round 89 locally fixed the committed overlay path so `.env.prod` remains the base env and `.env.nonprod-mock.example` only overlays mock payment values; focused deploy config checks passed. Current branch deploy/smoke remains unproven until the fixed workflow runs on ECS. | Pending deployment evidence | Rerun backend `deployment_lane=nonprod-mock-payment` after committing/pushing Round 89 and record it as non-production/mock-payment evidence. If MySQL credential access still fails, diagnose the persisted ECS MySQL user/volume state without printing secrets. Production-like evidence still requires real WeChat Pay production variables/key files. If GitHub-to-ECS artifact/API or remaining external base-image pulls become unstable again, use the documented no-new-paid-service fallback. |
