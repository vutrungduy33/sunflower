...
> Current as of 2026-06-04. This is the launch-readiness tracker for the MVP
...
| Deployment | Round 82 changed the workflow so a GitHub hosted job packages the deployment bundle and ECS deploy jobs download workflow artifacts instead of running `actions/checkout`. Run `26803892859` proved ECS-2 can download/extract/sync that bundle and download/load the backend image artifact, then failed production env validation because `WECHAT_PAY_MCH_ID` is missing. Round 87 changed backend/admin image builds to plain Docker CLI build/push/save; push run `26931880619` proved the hardened build path plus ECS backend artifact download/load, then failed at production `WECHAT_PAY_MCH_ID is required`. Round 88 backend-only `deployment_lane=nonprod-mock-payment` run `26932183311` proved strict readiness, backend-only dispatch, backend build/push/export/upload, ECS bundle/image artifact download/load, and nonprod lane guard, but failed during local backend deploy because MySQL app credentials from `.env.prod` could not access database `sunflower` as user `sunflower`. Round 89 fixed the committed overlay path so `.env.prod` remains the base env and `.env.nonprod-mock.example` only overlays mock payment values. Round 90 observed push run `26936286888` fail at the expected production payment validation blocker, then observed backend-only nonprod/mock run `26936565663` prove the overlay fix on ECS and reach backend recreation. That run exposed WeChat payment LOB schema drift; ECS-2 was repaired, backend became healthy, and post-recovery production smoke plus 8080 checks passed. The durable repo fix is Flyway V8. Round 90 push run `26937269296` for the V8 commit proved backend build/artifact/ECS download-load again, then failed before backend recreation at the expected production payment validation blocker. | Pending deployment evidence | For current-branch deployment evidence, rerun backend `deployment_lane=nonprod-mock-payment` now that V8 is on `main` and record it as non-production/mock-payment evidence, not real payment readiness. Production-like evidence still requires real WeChat Pay variables/key files. |
| Security / compliance | Secrets are local/ECS-owned; `.secrets/` ignored. Round 58 closed external backend `8080` by changing ECS-2 production `BACKEND_BIND_HOST` from `0.0.0.0` to `172.25.121.83`, recreating `sunflower-backend`, and verifying public 8080 is unusable while ECS-1 private upstream remains healthy. User-provided备案 domains now include `xiangrikui.cloud` and `sunflower.cloud`; local/public DNS currently resolves tested root/www/api/admin names to `198.18.x.x` reserved benchmarking addresses, and TLS probes did not return a usable certificate chain for the target hostnames. Free DV certificates such as Let's Encrypt can satisfy the technical HTTPS shape if DNS points to ECS-1, the chain is trusted, TLS is modern, renewal is automated, and the chosen API host is configured in the WeChat miniapp backend as a legal request domain. | Partially ready | Keep backend `8080` private after redeploys; point the chosen备案 API/admin hostnames to ECS-1, install/renew a trusted free certificate, verify TLS/certificate chain, then configure the WeChat legal request domain and record evidence. |
...
- `scripts/check_mvp_regression.sh`: passed in Round 86 on local `main` HEAD
  `158d894` with the default 5 non-production steps: backend tests
  ...
  config static checks. Production checks were skipped by default. An earlier
  Round 86 attempt failed only because `CURRENT-BRANCH-DEPLOYED.nextAction`
  lacked the explicit dispatch approval wording expected by the nonprod
  dispatch readiness guard; the evidence text was updated and the full
  aggregate regression was rerun successfully.
...
- `cd sunflower-admin-web && npm run lint`: passed in Round 86.
- `cd sunflower-admin-web && npm run test`: passed in Round 86, 24 tests across
  5 files.
- `cd sunflower-admin-web && npm run build`: passed in Round 86.
- `node scripts/check_admin_web_behavior_wiring.js`: passed in Round 86 with
  97 checks across 16 files.
- `node scripts/check_admin_web_external_qa_preflight.js`: passed in Round 86
  with 6 checks.
