...
| Deployment | Round 82 changed the workflow so a GitHub hosted job packages the deployment bundle and ECS deploy jobs download workflow artifacts instead of running `actions/checkout`. Run `26803892859` proved ECS-2 can download/extract/sync that bundle and download/load the backend image artifact, then failed production env validation because `WECHAT_PAY_MCH_ID` is missing. Round 83 dispatched backend-only `deployment_lane=nonprod-mock-payment` run `26804961943`, but backend image build failed before ECS deploy when Docker Buildx timed out pulling `moby/buildkit:buildx-stable-1` from Docker Hub. Round 87 changed backend/admin image builds to plain Docker CLI build/push/save and added a static guard against reintroducing Buildx actions. Push run `26931880619` proved the hardened build path plus ECS backend artifact download/load, then failed at production `WECHAT_PAY_MCH_ID is required`; current branch deploy/smoke remains unproven. | Blocked on actual deployment evidence | Provision real WeChat Pay production variables/key files and deploy production, or rerun backend `deployment_lane=nonprod-mock-payment` and record that it is not production payment evidence. If GitHub-to-ECS artifact/API or remaining external base-image pulls remain unstable, use the documented no-new-paid-service fallback: move release images/artifacts to Alibaba Cloud-side free resources such as ACR Personal Edition or an ECS-local signed artifact directory, then deploy from ECS over Alibaba Cloud/local network. |
