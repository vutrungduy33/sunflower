...
- 本 Stage 目标：将认证与用户模块从硬编码内存数据迁移到数据库，覆盖 `/api/auth/wechat/login`、`/api/auth/bind-phone`、`/api/users/me` 的真实读写，并按优先级完成一轮鉴权与登录链路加固（接入小程序 `wx.login -> code2session`）。
...
- 主要改动文件：
  ...
  - `sunflower-backend/src/main/java/com/sunflower/backend/modules/auth/WechatCode2SessionClient.java`
  ...
  - `sunflower-backend/src/test/java/com/sunflower/backend/modules/auth/WechatCode2SessionClientTests.java`
...
- 关键实现点：
  ...
  - 新增 `WechatCode2SessionClient`：`mock` 环境保留演示链路，`prod` 支持真实 `jscode2session` 交换 `openid`。
  - 小程序端 `wechatLogin` 改为默认调用 `wx.login()` 获取一次性 code，再调用 `/api/auth/wechat/login`；首页去除硬编码 `mvp_code`。
  - 小程序调用端统一默认 HTTPS，并将 `40100-40199` 统一识别为登录态失效，自动清理本地 token。
  - API 文档与 schema 文档同步到“签名+过期 token”语义。
  - 按 Stage Guard 要求同步触达 `scripts/sql/mvp_demo_seed.sql`，确保包含 persistence 变更的 PR 可通过 post-check。
  - 部署修复：`Deploy Backend To ECS` 通过 SSH 注入 `AUTH_TOKEN_SECRET`/微信登录相关环境变量，并在 `docker-compose` 显式透传到 backend 容器，避免 prod 启动缺参。
  - 部署健壮性修复：对 workflow 中读取的布尔/密钥变量做去空白、去引号与大小写标准化，并对异常布尔值回退兜底，避免 Secret 格式问题导致部署前置校验失败。
  - 部署健壮性补充：`AUTH_TOKEN_TTL_SECONDS` 增加数字校验与默认值回退（`7200`），并将 SSH action 改为显式错误处理，避免远端脚本因 `script_stop` 行为差异导致前置逻辑误退出。
  - 集成测试新增篡改签名与旧 token 格式拒绝用例，并把“不存在用户 token”期望收敛为 `40100/登录态无效`。
  - 2026-02-14 热修（review follow-up）：`app.auth.wechat.mock-fixed-openid` 默认值改为空，仅在显式设置 `WECHAT_MOCK_FIXED_OPENID` 时才固定账号，避免共享环境 mock 用户串号。
  - 新增 `WechatCode2SessionClientTests`，覆盖“固定 openid 关闭时按 `prefix + code` 生成”与“显式配置固定 openid 时固定返回”两条行为。
...
- 执行命令：
  ...
  - `cd sunflower-backend && mvn test -Dtest=WechatCode2SessionClientTests,MvpApiIntegrationTests`
- 结果：
  - 以上命令均通过。
  - `MvpApiIntegrationTests`：9/9 通过。
  - 全量测试：12/12 通过。
  - 2026-02-14 热修回归：`cd sunflower-backend && mvn test -Dtest=MvpApiIntegrationTests` 通过（10/10）。
  - 2026-02-14 review follow-up 回归：`cd sunflower-backend && mvn test -Dtest=WechatCode2SessionClientTests,MvpApiIntegrationTests` 通过（14/14）。

## 人工复核步骤

1. 启动后端（`dev` 配置），确认数据库中已有 `users` 与 `user_profiles` 数据。  
2. 小程序端进入首页触发 `wx.login()`，并调用 `POST /api/auth/wechat/login`（请求体携带临时 code），记录返回签名 token。  
...
7. 在 mock 环境不配置 `WECHAT_MOCK_FIXED_OPENID` 时，用不同 code 登录应映射为不同 openid；仅在显式配置 `WECHAT_MOCK_FIXED_OPENID` 后，不同 code 才应映射为同一 openid。  
...
- 当前已知风险：
  ...
  - 生产环境依赖 `WECHAT_APP_ID/WECHAT_APP_SECRET`，发布前需确认环境变量与微信后台配置一致。
- 下一 Stage 建议：
