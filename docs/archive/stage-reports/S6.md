...
- 关键实现点：
  ...
  - 小程序订单中心新增“改期/申请退款”入口，改期由固定“顺延 N 天”调整为弹窗日历选择入住/退房日期（仍保持原入住晚数约束）。
  - 小程序预订入口（预订页、房型详情页）日期选择改为 TDesign `t-calendar` 弹窗范围选择，统一“入住+退房”交互体验。
...
- 执行命令：`cd sunflower-miniapp && node --check pages/mvp/booking/index.js && node --check pages/mvp/room-detail/index.js && node --check pages/mvp/order-list/index.js`
- 结果：通过（3 个页面脚本语法检查通过）。
...
- 执行命令：`./scripts/stage_guard.sh post S6`
- 结果：通过（含 API contract guard 与 Stage 文档结构校验）。

## 人工复核步骤

1. 启动后端与小程序，确保小程序已登录且存在 `CONFIRMED` 状态订单。
2. 在预订页或房型详情页点击日期区域，弹出日历后选择入住/退房日期，确认后页面展示日期与晚数同步更新。
3. 在订单中心点击“改期”，弹出日历选择新的入住/退房日期（保持原入住晚数），确认订单状态变为“已改期”，入住离店日期同步变化。
4. 对 `CONFIRMED` 或 `RESCHEDULED` 订单点击“申请退款”，确认后状态变为“已退款”。
5. 在订单筛选栏切换“已改期”“已退款”，确认状态筛选与卡片状态色值一致。
6. （可选）调用 `GET /api/orders/{id}` 校验返回包含 `rescheduledAt/refundedAt/afterSaleReason` 字段。
7. 参照 `docs/M1-Integration-Test-Plan.md` 的 `TC-M1-12`、`TC-M1-13` 执行小程序售后入口与文档一致性回放。
8. 执行 `docs/M1-TC-M1-12-Manual-Checklist.md` 并按步骤补齐截图证据。
9. 在已登录状态下重新进入首页后再进入“订单中心”，确认仍为同一登录用户，订单列表不因首页重登而清空。
