...
const qaPath = path.join(rootDir, 'docs', 'Miniapp-Manual-QA.json');
...
function validateCheck(check) {
  ...
  if (!Array.isArray(check.relatedApis) || check.relatedApis.length === 0) {
    fail(`${check.id} must include at least one related API or capability`);
  }
...
function printSummary(checks) {
  ...
  console.log('[miniapp-qa] Required manual QA summary');
  console.log(`[miniapp-qa] total=${required.length} passed=${counts.passed} waived=${counts.waived} pending=${counts.pending} blocked=${counts.blocked}`);

  if (unresolved.length > 0) {
    console.log('[miniapp-qa] unresolved required checks:');
    ...
    console.log('[miniapp-qa] all required miniapp manual QA checks are passed or waived');
  }
...
if (strict && unresolved.length > 0) {
  fail(`miniapp manual QA is incomplete: ${unresolved.length} required check${unresolved.length === 1 ? '' : 's'} unresolved`);
}
